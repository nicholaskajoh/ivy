...
def run():
    ...
    droi = ast.literal_eval(os.getenv('DROI')) if use_droi else [(0, 0), (f_width, 0), (f_width, f_height), (0, f_height)]
    show_droi = ast.literal_eval(os.getenv('SHOW_DROI'))
    counting_line_position = os.getenv('COUNTING_LINE_POSITION')

    vehicle_counter = VehicleCounter(frame, detector, tracker, droi, show_droi, mcdf, mctf, di, counting_line_position)
    ...
    log_info('Processing started...', {
            'cat': 'COUNT_PROCESS',
            'counter_config': { 'di': di, 'mcdf': mcdf, 'mctf': mctf, 'detector': detector,
                                'tracker': tracker, 'use_droi': use_droi, 'droi': droi,
                                'show_droi': show_droi, 'clp': counting_line_position }})
    # main loop
